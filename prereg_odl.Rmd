...
Correlation assumptions : normality, linearity, level of measurement, related pairs
T-test assumptions      : normality, effect size, 
...
``` {r marcia-count_distribution, eval = FALSE}
...
```{r desc, eval = TRUE, include=FALSE}
...
```{r ttest_assump, include=FALSE}

# t-test intrinsic difference between female and non-female 
# effect size
...
for (i in seq_along(gender))
  ...
  for (j in seq_along(subscales))
    ...
    vec_ks <- ks.test(vec$mean, "pnorm", 
                      mean(vec$mean), sd(vec$mean))
...
# (marcia) I think we need to do a non-parametric test for female vs non-female - maybe the Kruskal-Wallis. 
